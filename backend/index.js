...
//Schema  creating fo rUser model
...
app.post('/signup',async(req,res)=>{
    ...
    let cart = {};//empty object/create empty cart
    for(let i = 0;i<300;i++){  //create user
        cart[i] = 0;
    }
        ...
        res.json({success:true,token})//success token
  
})

//Creating endpoint fo user login
...
const fetchUser = async(req,res,next) =>{
    ...
    if(!token){
        res.status(401).send({errors:"Please authonticate with valid token"})
        ...
        try{
           ...
           res.status(401).send({errors:"places authenticate using the valid token "});
        }
    }
...
//crating end point fo removing from cart
...
//creating to retriving cart data
